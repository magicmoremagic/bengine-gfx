...
std::mutex tga_write_mutex;

} // be::gfx::tex::()

//////////////////////////////////////////////////////////////////////////////
TgaWriter::TgaWriter(Log& log)
   : log_(log),
     z_(0) { }

//////////////////////////////////////////////////////////////////////////////
void TgaWriter::reset() {
   img_ = ConstImageView();
   z_ = 0;
}

//////////////////////////////////////////////////////////////////////////////
void TgaWriter::image(const ConstImageView& view, I32 z) noexcept {
   img_ = view;
   z_ = z;
}

//////////////////////////////////////////////////////////////////////////////
void TgaWriter::use_rle(bool rle) noexcept {
   rle_ = rle;
}

//////////////////////////////////////////////////////////////////////////////
Buf<UC> TgaWriter::write() {
   Buf<UC> result;
   std::error_code ec;
   result = write(ec);
   if (ec) {
...
//////////////////////////////////////////////////////////////////////////////
Buf<UC> TgaWriter::write(std::error_code& ec) noexcept {
   Buf<UC> result;
   ...
   if (img_.block_span() != img_.format().block_size() ||
       img_.line_span() != img_.dim().x * img_.block_span()) {
   ...
   {
      std::scoped_lock<std::mutex> lock(tga_write_mutex);
      ...
      if (!stbi_write_tga_to_func(
         ...
         }, &out, iv.dim().x, iv.dim().y, components, data)) {
...
//////////////////////////////////////////////////////////////////////////////
void TgaWriter::write(const Path& path) {
...
//////////////////////////////////////////////////////////////////////////////
void TgaWriter::write(const Path& path, std::error_code& ec) noexcept {
   // could save some memory allocations by duplicating the above except writing to file stream directly, but it shouldn't really matter much.
